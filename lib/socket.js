...
module.exports = function (app) {
    ...
    io.use(function(socket,next){
        ...
        if (users[socket.name].service) {
           socket.service=users[socket.name].service;
        }else{
            socket.service = "无服务";
        }
        next();
           ...
           var sp = packet.cmd;
            switch(sp) {
                case "online":
                    ...
                    break;
                case "chat":
                      socketShops[packet.data.id].send({cmd: "rec", data:{ id:id,message:packet.data.message,name:packet.data.name,tel:users[packet.data.name].tel}}); //与商家进行通信
                    break;
                case "ok":
